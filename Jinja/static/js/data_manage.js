...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fileSelectButton.addEventListener("click", function () {
        ...
        fileInput.addEventListener("change", function () {
            ...
            if (files.length > 0) {
                // Check if the selected file is of .csv type
                if (files[0].type === 'text/csv') {
                    // Process the file as needed
                    console.log('Selected file:', files[0].name);
                } else {
                    // If the selected file is not of .csv type, alert the user
    ...
    function handleFiles(files) {
        for (let i = 0; i < files.length; i++) {
            let file = files[i];
            if (file.type === 'text/csv') { // Check if it's a .csv file
                console.log('Selected file:', file.name);
                // Process the file as needed
            } else {
                alert('Please drop only .csv files.'); // Alert user if dropped file is not .csv
            }            
        }
    }
});
